...
public static partial class Commands
{
	...
	public static CommandDelegate Wobble(Ref<float> single, float amount, double duration) 
	{
		...
		float intervals = 3f; 
		float decay = Mathf.Log(100f * Mathf.Abs(amount));
	...
	public static CommandDelegate CircleAround(Ref<Vector3> position, Vector3 midpoint, float amount, double duration)
	{
		CheckDurationGreaterThanZero(duration);
		return CircleAround(position, midpoint, Vector3.up * amount, Vector3.right * amount, duration);
	}

	public static CommandDelegate CircleAround(Ref<Vector3> position, Vector3 midpoint, Vector3 normal, Vector3 tangent, double duration)
	{
		CheckDurationGreaterThanZero(duration);

		return Commands.Duration(
			(t) => {
				float angle = ((float)t) * Mathf.PI * 2f;
				float x = Mathf.Cos(angle);
				float y = Mathf.Sin(angle);

				position.Value = midpoint + normal * x + tangent * y;
			}, duration
		);
	}
}
